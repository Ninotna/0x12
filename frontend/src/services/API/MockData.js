...
const USER_MAIN_DATA = [
   ...
   * @param {number|string} userId - L'ID de l'utilisateur (converti en number)
  ...
  export function getMockUserActivity(userId) {
    const numericUserId = Number(userId);
    return USER_ACTIVITY.find((activity) => activity.userId === numericUserId) || null;
  }
  ...
  export function getMockUserAverageSessions(userId) {
    const numericUserId = Number(userId);
    return USER_AVERAGE_SESSIONS.find((session) => session.userId === numericUserId) || null;
  }
  ...
  export function getMockUserPerformance(userId) {
    const numericUserId = Number(userId);
    return USER_PERFORMANCE.find((performance) => performance.userId === numericUserId) || null;
  }
